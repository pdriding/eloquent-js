...
async function fileContents(files) {
  let list = "";
  console.log(3333, typeof files);
  if (typeof files === "string") {
    list = await textFile(files);
  } else {
    await Promise.all(
      files.map(async (fileName) => {
        let test = await textFile(fileName);
        list += test;
      })
    );
  }
...
//   let logFileList = await fileContents("camera_logs.txt");

//   let test2 = await fileContents(logFileList);
...
//   test2.forEach((a) => {
//     let hmmm = new Date(Number(a));
//     if (hmmm.getDay() === day) {
//       let hour = hmmm.getHours();
//       hourArray[hour]++;
//     }
//   });
...
// async function activityTable(day) {
//   let hourArray = [];
//   for (i = 0; i < 24; i++) hourArray[i] = 0;

//   let logFileList = await textFile("camera_logs.txt");

//   for (a of logFileList.split("\n")) {
//     let b = await textFile(a);
//     for (c of b.split("\n")) {
//       let d = new Date(Number(c));
//       if (d.getDay() === day) {
//         let hour = d.getHours();
//         hourArray[hour]++;
//       }
...
function fetchData() {
  return new Promise((res, rej) => {
    let success = textFile("camera_logs.txt");
    if (success) {
      res(success);
    } else {
...
function activityTable(day) {
  ...
  return textFile("camera_logs.txt")
    .then((data) => {
      let test = data.split("\n");
      return Promise.all(test.map((a) => textFile(a)));
    })
    .then((data) => {
      let list = "";
      for (a of data) {
...
function Promise_all(promises) {
  return new Promise((resolve, reject) => {
    ...
    let remainingPromises = promises.length; // Tracks how many promises are still pending

    if (remainingPromises === 0) {
      resolve(results); // Resolve immediately if no promises are provided
      return;
    }
    ...
    promises.forEach((promise, i) => {
      promise
        .then((value) => {
          results[i] = value; // Store the resolved value at the correct index
          remainingPromises--; // Decrement the counter
          console.log(
            "Remaining:",
            remainingPromises,
            "Resolved Value:",
