...
async function activityTable2(day) {
  let table = new Array(24).fill(0);

  for (let f of (await textFile("camera_logs.txt")).split("\n")) {
    for (let a of (await textFile(f)).split("\n")) {
...
async function activityTable1(day) {
  let table = [];
  for (let i = 0; i < 24; i++) {
    table[i] = 0;
    ...
    for (f of files) {
      let dateFile = await textFile(f);
      for (a of dateFile.split("\n")) {
        let number = Number(a);
        let date = new Date(number);
        let check = date.getDay(day);
...
// activityTable1(1).then((table) => console.log(activityGraph(table)));

// function activityTable(day) {
//   let table = new Array(24).fill(0);

//   const myPromise = new Promise((resolve, reject) => {
//     let logFileList = textFile("camera_logs.txt");
//     if (logFileList) {
//       resolve(logFileList);
//     } else {
//       reject(2);
//     }
//   });
//   let test2 = myPromise.then((a) => {
//     let f = a.split("\n");
//     let test = Promise.all(f.map((b) => textFile(b)));
//     let fred = test.then((p) => {
//       let maybe = p.join("+").split("\n");
//       for (z of maybe) {
//         let date = new Date(Number(z));
//         if (date.getDay() === day) {
//           table[date.getHours()]++;
//         }
//       }
...
function activityTable3(day) {
  ...
  return textFile("camera_logs.txt")
    .then((files) => {
      return Promise.all(
        files.split("\n").map((a) => {
          return textFile(a).then((timeStamp) => {
            for (a of timeStamp.split("\n")) {
              let date = new Date(Number(a));
              if (date.getDay() === day) {
                table[date.getHours()]++;
              }
...
// activityTable3(6).then((table) => console.log(activityGraph(table)));

// function Promise_all(promises) {
//   return new Promise((resolve, reject) => {
//     let test = new Array(promises.length);
//     let resolvedCount = 0;

//     promises.forEach((a, i) => {
//       a.then((b) => {
//         test[i] = b;
//         resolvedCount++;
...
//       }).catch((error) => {
//         console.log(error);
//         reject(error); // Reject the outer promise if any promise rejects
//       });
//     });
//     // Edge case: Handle the case where the input array is empty
  ...
  return new Promise((resolve, reject) => {
    ...
    promises.forEach((a, i) => {
      a.then((b) => {
        ...
        if (test.length === promises.length) {
          resolve(test);
        }
      }).catch((error) => {
        console.log(error);
